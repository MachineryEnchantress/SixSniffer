...
const char STR_CTW[] = {"Convert to golist ?"};
const char STR_RAW[] = {"Removing a golist ?"};
...
static int read_SD_txt_row(String path);
static String read_SD_txt_row(String path, int row);
static void delline_SD_txt_row(String path, int row);

static void wifi_event_cb();
static void buletooth_event_cb();
...
MenuBar::lv_menu_config_t _cfg[7] = {
    ...
    {.name = "Bluetooth", .img = (void *)&bluetooth, .event_cb = buletooth_event_cb},
...
public:
    ...
    void create(lv_obj_t *parent = nullptr)
    {
        ...
        _listCont = lv_list_create(_listCont == nullptr ? lv_scr_act() : parent, NULL);
...
public:
    ...
    typedef enum
    {
        LIST_ALL_EQUIPMENT = 0,
        LIST_SUSPICIOUS_DEVICE,
        LIST_GOLIST
    };
   ...
   // Create an interface
    void create(tabview_cfg_t *cfg, uint8_t count, exit_cb cb, lv_obj_t *parent = nullptr)
    {
        static lv_style_t tablStyle;
        lv_style_init(&tablStyle);
        lv_style_set_radius(&tablStyle, LV_OBJ_PART_MAIN, 0);
        lv_style_set_bg_color(&tablStyle, LV_OBJ_PART_MAIN, LV_COLOR_GRAY);
        lv_style_set_bg_opa(&tablStyle, LV_OBJ_PART_MAIN, LV_OPA_0);
        lv_style_set_border_width(&tablStyle, LV_OBJ_PART_MAIN, 0);
        lv_style_set_border_opa(&tablStyle, LV_OBJ_PART_MAIN, LV_OPA_50);
        lv_style_set_text_color(&tablStyle, LV_OBJ_PART_MAIN, LV_COLOR_WHITE);
        lv_style_set_image_recolor(&tablStyle, LV_OBJ_PART_MAIN, LV_COLOR_WHITE);
        ...
        _tabCont = lv_page_create(parent, NULL);
        lv_obj_set_size(_tabCont, LV_HOR_RES, LV_VER_RES - 30);
        lv_obj_align(_tabCont, NULL, LV_ALIGN_CENTER, 0, 0);
        lv_obj_add_style(_tabCont, LV_OBJ_PART_MAIN, &tablStyle);
        lv_page_set_edge_flash(_tabCont, true);
        lv_page_set_scrollable_fit2(_tabCont, _LV_FIT_LAST, LV_FIT_TIGHT);
        lv_page_set_scrlbar_mode(_tabCont, LV_SCRLBAR_MODE_OFF);
        _count = count;

        _tab = new lv_obj_t;
        _cfg = new tabview_cfg_t[count];

        memcpy(_cfg, cfg, sizeof(tabview_cfg_t) * count);

        lv_obj_t *la1 = lv_label_create(_tabCont, NULL);
        lv_label_set_text(la1, cfg->name);
        lv_obj_align(la1, NULL, LV_ALIGN_IN_TOP_MID, 0, 0);
        _tab = lv_imgbtn_create(_tabCont, NULL);
        lv_imgbtn_set_src(_tab, LV_BTN_STATE_RELEASED, &btn_scan);
        lv_imgbtn_set_src(_tab, LV_BTN_STATE_PRESSED, &btn_scan);
        lv_imgbtn_set_src(_tab, LV_BTN_STATE_CHECKED_RELEASED, &btn_scan);
        lv_imgbtn_set_src(_tab, LV_BTN_STATE_CHECKED_PRESSED, &btn_scan);
        lv_obj_set_click(_tab, true);

        lv_obj_align(_tab, la1, LV_ALIGN_OUT_RIGHT_MID, 5, 0);
        lv_obj_set_event_cb(_tab, __tabview_event_cb);

        _exitBtn = lv_imgbtn_create(_tabCont, NULL);
        lv_imgbtn_set_src(_exitBtn, LV_BTN_STATE_RELEASED, &iexit);
        lv_imgbtn_set_src(_exitBtn, LV_BTN_STATE_PRESSED, &iexit);
        lv_imgbtn_set_src(_exitBtn, LV_BTN_STATE_CHECKED_RELEASED, &iexit);
        lv_imgbtn_set_src(_exitBtn, LV_BTN_STATE_CHECKED_PRESSED, &iexit);
        ...
        lv_obj_align(_exitBtn, _tabCont, LV_ALIGN_IN_TOP_LEFT, 10, 0);
        lv_obj_set_event_cb(_exitBtn, __tabview_event_cb);

        //Create tabview
        _tv = lv_tabview_create(_tabCont, NULL);

        lv_obj_align(_tv, _tabCont, LV_ALIGN_CENTER, 0, 0);
        lv_obj_set_size(_tv, LV_HOR_RES, 160);

        _subtab = new lv_obj_t *[3];
        _subtab[LIST_ALL_EQUIPMENT] = lv_tabview_add_tab(_tv, "results");
        _subtab[LIST_SUSPICIOUS_DEVICE] = lv_tabview_add_tab(_tv, "Sus.");
        _subtab[LIST_GOLIST] = lv_tabview_add_tab(_tv, "golist");

        _tablist = new lv_obj_t *[3];

        lv_obj_t *list_btn;
        for (int i = 0; i < 3; i++)
        {
            _tablist[i] = lv_list_create(_subtab[i], NULL);
            lv_obj_align(_tablist[i], NULL, LV_ALIGN_IN_BOTTOM_MID, 0, -10);
            lv_obj_set_size(_tablist[i], 200, 100);
        }
    ...
    void align(const lv_obj_t *base, lv_align_t align, lv_coord_t x = 0, lv_coord_t y = 0)
    {
        lv_obj_align(_tabCont, base, align, x, y);
    }

    void hidden(bool en = true)
    {
        lv_obj_set_hidden(_tabCont, en);
    }
    //Clean up pages and records
    void clean(void)
    {
        lv_list_clean(_tablist[LIST_ALL_EQUIPMENT]);
        lv_list_clean(_tablist[LIST_SUSPICIOUS_DEVICE]);
    ...
    static void __golist_refresh(void)
    {
        String str;
        int golist_row = read_SD_txt_row(GOLIST_PATH);
        Serial.printf("golist row:%d\n", golist_row);
        lv_list_clean(_tablist[LIST_GOLIST]);
        for (int i = 0; i < golist_row; i++)
        {
            str = read_SD_txt_row(GOLIST_PATH, i);
            if (!str.isEmpty())
            {
                Serial.print(str.c_str());
                add(LIST_GOLIST, str.c_str());
            }
        }
    }
//Comparison of last time with this time
    static void AnalyzeDevicesToSus(void)
    {
        String str;
        File file;
        char *str_ptr;

        int result_row = read_SD_txt_row(RESULTS_PATH);
        int cache_row = read_SD_txt_row(CACHE_PATH);

        Serial.printf("result_row %d\n", result_row);
        Serial.printf("cache_row %d\n", cache_row);

        if (cache_row)
        {
            for (int i = 0; i < cache_row; i++)
            {
                str = read_SD_txt_row(CACHE_PATH, i);
                file = SD.open(RESULTS_PATH, FILE_READ);
                if (file.find(str.c_str() /* , str.length() */))
                { // Duplicate address found, check golist, add to suspect list
                    file.close();
                    file = SD.open(GOLIST_PATH, FILE_READ);
                    if (!file.find(str.c_str() /* , str.length() */))
                    {
                        add(LIST_SUSPICIOUS_DEVICE, str.c_str());
                        Serial.printf("Suspicious devices:%s\n", str.c_str());
                    }
                }
                file.close();
            }
        ...
        str_ptr = (char *)heap_caps_malloc(sizeof(char) * (file.size() + 1), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (str_ptr != nullptr)
        {
            int i = 0;
            file.seek(0);
            ...
            *(str_ptr + i) = '\0';
            ...
            file = SD.open(CACHE_PATH, FILE_WRITE);
            file.seek(0);
            file.print(str_ptr);
            file.flush();
            file.close();
        ...
        heap_caps_free(str_ptr);
    }

    static void __creatMsgBox_event_handler(lv_obj_t *obj, lv_event_t event)
    {
        if (event == LV_EVENT_VALUE_CHANGED)
        {
            if (lv_msgbox_get_active_btn_text(obj) == "Close")
            {
            }
            else if (lv_msgbox_get_active_btn_text(obj) == "Move")
            {
                delline_SD_txt_row(GOLIST_PATH, (int)lv_obj_get_user_data(obj));
                __golist_refresh();
            }
            else
            {
                String str;
                if (lv_msgbox_get_active_btn_text(obj) == "Apply")
                {
                    str = read_SD_txt_row(RESULTS_PATH, (int)lv_obj_get_user_data(obj));
                }
                else
                {
                    // str = read_SD_txt_row(CACHE_PATH, (int)lv_obj_get_user_data(obj));
                    str = (char *)lv_obj_get_user_data(obj);
                }

                File file = SD.open(GOLIST_PATH, FILE_WRITE);
                if (file)
                {
                    file.seek(file.size());
                    file.printf("%s\n", str.c_str());
    ...
    static void __creatMsgBox_sus_event_cb(_lv_obj_t *obj, lv_event_t event)
    {
        ...
        if (event == LV_EVENT_CLICKED)
        {
            ...
            lv_msgbox_set_text(mbox1, (char *)obj->user_data);

            lv_msgbox_add_btns(mbox1, btns);
            lv_obj_set_size(mbox1, 200, 200);
            lv_obj_set_user_data(mbox1, (lv_obj_user_data_t)lv_list_get_btn_text(obj));
            lv_obj_set_event_cb(mbox1, __creatMsgBox_event_handler);
            lv_obj_align(mbox1, NULL, LV_ALIGN_CENTER, 0, 0);
        }
    }
    static void __creatMsgBox_result_event_cb(_lv_obj_t *obj, lv_event_t event)
    {
        static const char *btns[] = {"Apply", "Close", ""};

        if (event == LV_EVENT_CLICKED)
        {
            int index = lv_list_get_btn_index(_tablist[LIST_ALL_EQUIPMENT], obj);
            ...
            lv_msgbox_add_btns(mbox1, btns);
            lv_obj_set_size(mbox1, 200, 200);
            lv_obj_set_user_data(mbox1, (lv_obj_user_data_t)index);
            lv_obj_set_event_cb(mbox1, __creatMsgBox_event_handler);
            lv_obj_align(mbox1, NULL, LV_ALIGN_CENTER, 0, 0);
        }
    }
    static void __creatMsgBox_golist_event_cb(_lv_obj_t *obj, lv_event_t event)
    {
        static const char *btns[] = {"Move", "Close", ""};

        if (event == LV_EVENT_CLICKED)
        {
            int index = lv_list_get_btn_index(_tablist[LIST_GOLIST], obj);
    ...
    static void add(uint8_t whichlist, const char *txt, void *imgsrc = (void *)LV_SYMBOL_BLUETOOTH)
    {
        lv_obj_t *btn = lv_list_add_btn(_tablist[whichlist], imgsrc, txt);
        if (whichlist == LIST_GOLIST)
        {
            btn->user_data = (lv_obj_user_data_t)STR_RAW;
            lv_obj_set_event_cb(btn, __creatMsgBox_golist_event_cb);
        }
        else if (whichlist == LIST_ALL_EQUIPMENT)
        {
            btn->user_data = (lv_obj_user_data_t)STR_CTW;
            lv_obj_set_event_cb(btn, __creatMsgBox_result_event_cb);
        }
        else
        {
            btn->user_data = (lv_obj_user_data_t)STR_CTW;
            lv_obj_set_event_cb(btn, __creatMsgBox_sus_event_cb);
        }
    }

    static void __tabview_event_cb(lv_obj_t *obj, lv_event_t event)
    {
        if (event == LV_EVENT_SHORT_CLICKED)
        {
            Serial.println("LV_EVENT_SHORT_CLICKED");
            if (obj == _tabview->_exitBtn)
            {
                if (_tabview->_exit_cb != nullptr)
                {
                    _tabview->_exit_cb();
                    return;
                }
        ...
        {
            Serial.println("LV_EVENT_VALUE_CHANGED");
            for (int i = 0; i < _tabview->_count; i++)
            {
                lv_obj_t *tv = _tabview->_tab;
                if (obj == tv)
                {
                    if (_tabview->_cfg[i].cb != nullptr)
                    {
                        _tabview->_cfg[i].cb(i, true);
                    }
                    return;
                }
            }
    ...
    void setStatus(uint8_t index, bool en)
    {
        ...
        lv_obj_t *tab = _tab;
        const void *dst = en ? &on : &off;
        lv_imgbtn_set_src(tab, LV_BTN_STATE_RELEASED, &btn_scan);
        lv_imgbtn_set_src(tab, LV_BTN_STATE_PRESSED, &btn_scan);
        lv_imgbtn_set_src(tab, LV_BTN_STATE_CHECKED_RELEASED, &btn_scan);
        lv_imgbtn_set_src(tab, LV_BTN_STATE_CHECKED_PRESSED, &btn_scan);
    }

private:
    static Tabview *_tabview;
    lv_obj_t *_tabCont = nullptr;
    uint8_t _count;
    lv_obj_t *_tab = nullptr;
    tabview_cfg_t *_cfg = nullptr;
    lv_obj_t *_exitBtn = nullptr;
    lv_obj_t *_tv = nullptr;
    Task *refresh_task = nullptr;
    lv_obj_t **_subtab = nullptr; // results

    static lv_obj_t **_tablist;

    exit_cb _exit_cb = nullptr;
};
Tabview *Tabview::_tabview = nullptr;
lv_obj_t **Tabview::_tablist = nullptr;
...
static Tabview *tabview = nullptr;

static char ssid[64], password[64];
/*****************************************************************
 *
 *          SD card simplified operation
 *
 */
...
static void delline_SD_txt_row(String path, int row)
{
    const int row_string = 27;
    char *cache;

    File file = SD.open(path, FILE_READ);
    if (file)
    {
        file.seek((row + 1) * row_string);
        //Apply for a piece of memory to store data from PSRAM
        cache = (char *)heap_caps_malloc(sizeof(char) * ((file.size() - ((row + 1) * row_string) + 1)), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        int size = file.size() - (((row + 1) * row_string) + 1);
        int i = 0;
        while (file.available())
        {
            *(cache + i) = file.read();
...
void scanEndedCB(NimBLEScanResults results)
{
    ...
    tabview->AnalyzeDevicesToSus();
}

void ble_sw_event_cb(uint8_t index, bool en)
{
    switch (index)
    {
    case 0:
        if (en)
        {
            ...
            if (pBLEScan->isScanning() == false)
            {
                ...
                pBLEScan->start(30, scanEndedCB, false);
            }
        }
        else
        {
...
void ble_list_add(char *str)
{
    if (tabview != nullptr)
    {
        tabview->add(Tabview::LIST_ALL_EQUIPMENT, str);
    }
}
...
static void buletooth_event_cb()
{
    Tabview::tabview_cfg_t cfg = {"BLE Scan", ble_sw_event_cb};
    tabview = new Tabview;
    tabview->create(&cfg, 1, []()
                    {
        delete tabview;
        tabview = nullptr;
        menuBars.hidden(false); });
    tabview->align(bar.self(), LV_ALIGN_OUT_BOTTOM_MID);
    tabview->setStatus(0, WiFi.isConnected());
}
